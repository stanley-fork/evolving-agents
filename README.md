...
<a href="https://evolvingagentslabs.github.io/demo/"><img src="doc/assets/manual/09-desk.jpg" alt="The desk: every box an agent, the lines between them the handoffs, and one panel inspecting whatever is selected" width="100%"></a>

<sub><b><a href="https://evolvingagentslabs.github.io/demo/">Try the desk →</a></b> Every box is an agent. The lines between them are the handoffs, and the dot on a line is what actually moved. One panel inspects whatever you select — or you drag <code>INSPECTOR</code> onto it and let an agent do the reading, and <b>every sentence it produces carries the address it read</b>. When it has nothing to read it says <i>unknown</i>, and the desk draws that differently from an answer.<br><br>It lands on <b>coclea-sr</b>: two chains, the same six agents, both green, one wrong in every number — separated only by a gate declared before the run. The second scope is <b>hemo-verified</b>, where no closed form exists, so the judge itself is measured. <b>The orchestration is simulated</b> — no core, no model, nothing stored — but the numbers those two scopes show are read out of the projects' own artifacts, and <a href="scripts/check-demo-provenance.py"><code>check-demo-provenance.py</code></a> fails the build if any of them stops matching. To re-derive them yourself, byte by byte, in your own browser: <b><a href="https://evolvingagentslabs.github.io/verify/">the verification page</a></b>. That page is not a demo and does not try to be.</sub>
