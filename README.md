...
<a href="https://evolvingagentslabs.github.io/demo/"><img src="doc/assets/manual/09-desk.jpg" alt="A bundle of flows coiled around a time axis, with the agents riding the strands they hold" width="100%"></a>

<sub><b><a href="https://evolvingagentslabs.github.io/demo/">Open the demo →</a></b> <b>One axis, and it is time.</b> Every flow is a strand wound around it, so they share a band of screen and separate in <b>depth</b> rather than in height — what is bright and thick is at the front, what is faint is behind the bundle. <b>Drag to turn it.</b> Turning is how you pay attention: a different strand comes forward without anything moving out of the way.<br><br><b>The agents ride the strands they hold.</b> A circle is an agent with the thought in hand; where a strand carries a hollow mark instead, nobody is holding it — that is the thing in transit between two of them. <code>INSPECTOR</code>, a system agent with one tool (read), attaches to any strand and rides it, and everything it says carries the address it read.<br><br><b>Two motions, two meanings.</b> Everything drifts left always, because the clock is running — which is unconditionally true, and since the strands twist along the axis that drift also turns the bundle. Light travels a strand <i>only</i> where a step is open right now. And <b>turning is a claim</b>: the panel says which strand is in front, why, and where it read that — a rotation the surface cannot justify is one it refuses to make.<br><br>Zooming out aggregates rather than shrinks: below forty-six pixels a turn the coil stops being drawn and says so, because what you would see is the sampling and not the shape. Four scopes, all real — <b>coclea-sr</b> (a gate that measured 2.592e-4 against a tolerance of 1.0e-4), <b>hemo-verified</b> (no closed form, so the judge itself is measured at 0.9056), and two that each carry a flow which is green and wrong. <b>The orchestration is simulated</b>; the numbers come from the projects' own artifacts, and <a href="scripts/check-demo-provenance.py"><code>check-demo-provenance.py</code></a> fails the build if any stops matching. To re-derive them yourself: <b><a href="https://evolvingagentslabs.github.io/verify/">the verification page</a></b>.</sub>
