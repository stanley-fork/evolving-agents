...
| [**The platform plan**](PLAN-PLATFORM.md) | A product brainstorm mapped onto this repository as it exists on disk: four of its five phases already have a predecessor, two have results that contradict it, and every stage is gated on a measurement · [es](es/PLAN-PLATFORM.md) |
| [**The validation cases**](PLAN-PLATFORM-CASES.md) | What each stage of the platform plan is validated on — real work, not demos, and three of them run against **this repository's own enforced house rules** · [es](es/PLAN-PLATFORM-CASES.md) |
